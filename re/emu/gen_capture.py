...
def main():
    ...
    # The generator opens an ofstream for the output file; under emulation that open
    # FAILS (failbit set) so the first stream-state gate at 0x100fc003
    #   test byte [stream_state],6 ; je 0x100fc0e3(build)
    # falls through to a -2 early return (0x100fc0d9). Force it to the build path so
    # the VHDL gets assembled in memory (our append hooks capture it; the eventual
    # ofstream writes are harmless no-ops). je rel32 (0f84 da000000) -> jmp (e9 db000000) + nop.
    uc.mem_write(0x100fc003, bytes([0xe9,0xdb,0x00,0x00,0x00,0x90]))
    # second stream-state gate at 0x100fc11c (je 0x100fc1fc, else -3). Same fix.
    uc.mem_write(0x100fc11c, bytes([0xe9,0xdb,0x00,0x00,0x00,0x90]))
    print("patched stream-state gates 0x100fc003 + 0x100fc11c je->jmp (force build path)")

    bigDataObj = u32(crop+8)   # display object (builder fcn.101a8370 uses [crop+8])
    print("\n--- data_emu gen: bigDataObj=[crop+8]=0x%x ---" % bigDataObj)
