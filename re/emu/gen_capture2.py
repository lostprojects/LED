...
# positional args = non-flag argv (flags like --shortcut/--mode are handled separately)
_pos = [a for a in sys.argv[1:] if not a.startswith("--")]
# strip VALUE of any value-taking flag (--mode N, --argoff N) from positionals
for _f in ("--mode", "--argoff"):
    if _f in sys.argv:
        _i = sys.argv.index(_f)
        _mv = sys.argv[_i+1] if _i+1 < len(sys.argv) else None
        if _mv in _pos: _pos.remove(_mv)
DLL = _pos[0] if len(_pos) > 0 else "/home/muse/Desktop/LED/re/dll/CLTDevice.dll"
RCVP = _pos[1] if len(_pos) > 1 else \
...
OUTWRITES = {}    # handle -> bytearray of all WriteFile payloads to that handle
OUTPUT_HANDLES = set()   # handles for .vhd/.txt output files
CREATEFILE_CALLS = []    # filenames passed to CreateFileW (trace)
trace_imports = True
...
def make_shims(uc, name_by_hookaddr):
    ...
    def shim(name):
        ...
        if name == "CreateFileW" or name == "CreateFileA":
            NEXT_HANDLE += 1
            # read the filename to dispatch: .vhd/.txt output (empty, capture writes)
            # vs default (serve the rcvp). a[0] = LPCWSTR/LPCSTR filename.
            fnamep = a[0]; fname = ""
            try:
                if name == "CreateFileW":
                    raw = bytes(rd(fnamep, 600))
                    z = raw.find(b"\x00\x00")
                    fname = raw[:z].decode("utf-16-le", "ignore")
                else:
                    raw = bytes(rd(fnamep, 300)); fname = raw[:raw.find(b"\x00")].decode("latin1","ignore")
            except Exception:
                pass
            low = fname.lower()
            if low.endswith(".vhd") or low.endswith(".txt") or "data_emu" in low:
                FILES[NEXT_HANDLE] = FakeFile(b"")          # output target
                OUTPUT_HANDLES.add(NEXT_HANDLE)
                print("  [CreateFileW] OUTPUT '%s' -> handle 0x%x" % (fname, NEXT_HANDLE))
            else:
                FILES[NEXT_HANDLE] = FakeFile(rcvp_bytes)
                print("  [CreateFileW] '%s' -> handle 0x%x (serve rcvp)" % (fname, NEXT_HANDLE))
            CREATEFILE_CALLS.append(fname)
            return NEXT_HANDLE
        ...
        if name in ("WriteFile",):
            try:
                payload = bytes(rd(a[1], a[2]))
                WRITES.append(payload)
                OUTWRITES.setdefault(a[0], bytearray())
                OUTWRITES[a[0]] += payload
            except: pass
            if a[3]: w32(a[3], a[2])
            return 1
        if name == "CloseHandle":
        ...
        if name in ("EncodePointer","DecodePointer"):
            # raw-0 slots are uninitialized fn-pointer globals (headless: never set by
            # CRT static init). Real CRT idiom is `p=DecodePointer(g); if(p) p();` —
            # so an uninit slot must decode to 0 ("absent") or the CRT calls a bogus
            # pointer (= cookie) and faults. Special-case 0 both ways.
            if a[0] == 0:
                return 0
            ck = u32(0x102785d4)
            return a[0] ^ ck
...
def main():
    ...
    IMPLOG = deque(maxlen=30)   # ring of recent (name, argc) for crash diagnosis
    def hook_code(uc, address, size, ud):
        if HOOK_BASE <= address < HOOK_BASE+HOOK_SIZE and address != END_ADDR:
            ...
            IMPLOG.append((nm, argc_by_hook.get(address,0), cdecl_by_hook.get(address)))
            sp = uc.reg_read(UC_X86_REG_ESP)
    ...
    def hook_mem_invalid(uc, access, address, size, value, ud):
        ...
        if access == 21 or address < 0x10000:   # bad fetch OR null-page deref = real bug
            stk = struct.unpack("<8I", uc.mem_read(sp,32))
            print("    [FAULT acc=%d] target=0x%x EIP=0x%x ESP=0x%x stack=%s" %
                  (access, address, eip, sp, [hex(x) for x in stk]))
            print("    last blocks: %s" % [hex(x) for x in list(blk["hist"])])
            print("    last imports (name,argc,cdecl): %s" % list(IMPLOG))
            dump_trace()
            return False
        page = address & ~0xfff
        try:
    ...
    # ---- CRT stdio/lowio table init (Plan A: make ofstream open succeed) ----
    # _getstream (0x101e83ab) loops bounded by _nstream@0x1028b904 (was 0 -> returns
    # NULL -> EMFILE before CreateFileW). Give it a real __piob@0x1028b900 FILE*
    # array + a nonzero _nstream so it can allocate a FILE slot.
    NSTREAM = 64
    piob = halloc(NSTREAM*4); uc.mem_write(piob, b"\x00"*(NSTREAM*4))
    uc.mem_write(0x1028b900, struct.pack("<I", piob))      # __piob
    uc.mem_write(0x1028b904, struct.pack("<I", NSTREAM))   # _nstream
    # _alloc_osfhnd (0x101fa775) indexes __pioinfo@0x10289f60 (array of ptrs to
    # 32x 0x40-byte ioinfo chunks). Seed chunk[0] zeroed so a free fd is found and
    # the CreateFileW handle gets stored into ioinfo.osfhnd(+0)/osfile(+4).
    chunk0 = halloc(32*0x40); uc.mem_write(chunk0, b"\x00"*(32*0x40))
    uc.mem_write(0x10289f60, struct.pack("<I", chunk0))    # __pioinfo[0]
    print("CRT lowio: __piob=0x%x _nstream=%d __pioinfo[0]=0x%x" % (piob, NSTREAM, chunk0))
    trace_imports = True
    ...
    # Inspect crop's loader vtable[0x48] (the real file loader InitFromFile tail-calls)
    cvt = u32(crop)
    loader = u32(cvt + 0x48)
    print("crop vtable=0x%x  loader(vtable[0x48])=0x%x" % (cvt, loader))

    # early diag: watch the +0xd650 flag-setter during init too
    if ("--diag" in sys.argv):
        def _d_setflag_early(uc, address, size, ud):
            edi = uc.reg_read(UC_X86_REG_EDI)
            ecx = uc.reg_read(UC_X86_REG_ECX)
            print("    [init: 0x10095f95 SET byte[0x%x+0xd650]=%d (ecx=0x%x)]"
                  % (edi, (ecx>>16)&1, ecx))
        uc.hook_add(UC_HOOK_CODE, _d_setflag_early, begin=0x10095f95, end=0x10095f95)

    USE_SHORTCUT = ("--shortcut" in sys.argv)
    if USE_SHORTCUT:
        # Legacy shortcut: memory-deserialize the .rcvp + call the display-builder
        # directly. This succeeds but leaves some display sub-objects unbuilt, so the
        # generator null-derefs at 0x100c67d0. Kept for comparison only.
        ...
        if ("--diag" in sys.argv):
            cc = u32(crop+8)
            def _hx(base, off, n):
                try: return bytes(uc.mem_read(base+off, n)).hex()
                except: return "??"
            print("  [codec dump] codec=0x%x" % cc)
            for o in range(0, 0x100, 16):
                print("    +%04x: %s" % (o, _hx(cc,o,16)))
            print("    +d640: %s" % _hx(cc,0xd640,32))
            print("    +d660: %s" % _hx(cc,0xd660,32))
    else:
        # Real full init: _CLTReceiverRcvParamInitFromFile@4(path) -> singleton ->
        # crop.vtable[0x48](path) = the actual ifstream loader. Now that CRT file I/O
        # works, this opens the .rcvp (CreateFileW serves rcvp_bytes for any non
        # .txt/.vhd name), parses it, AND builds every display sub-object (so the
        # generator no longer hits the null-deref). Provide BOTH a wide and a narrow
        # path so it works regardless of how the loader treats the arg.
        pstr = "C:\\rcv\\config.rcvp"
        wp = halloc(len(pstr)*2 + 2); uc.mem_write(wp, pstr.encode("utf-16-le") + b"\x00\x00")
        ap = halloc(len(pstr) + 1);   uc.mem_write(ap, pstr.encode("latin1") + b"\x00")
        init_fn = EXPORTS["_CLTReceiverRcvParamInitFromFile@4"]
        rI = call(init_fn, [wp], "InitFromFile(wide path)")
        print("  InitFromFile(wide) -> %s" % (hex(rI) if rI is not None else "FAULT"))
        if rI is None or (rI & 0xffffffff) == 0xe0830001 or (rI & 0x80000000):
            rI2 = call(init_fn, [ap], "InitFromFile(narrow path)")
            print("  InitFromFile(narrow) -> %s" % (hex(rI2) if rI2 is not None else "FAULT"))
        # re-fetch codec after the real load (the loader may rebuild it)
        codec = u32(crop + 8)
        print("  codec after InitFromFile = 0x%x" % codec)

    # 2c) data_emu generator = singleton.vtable[0x54] = fcn.100fbe20(this=singleton,
    #     bigDataObj). It BUILDS the filled VHDL in a std::string then writes it via
    #     ofstream (NOT captured by our WriteFile shim -> 0 bytes). So instead we hook
    #     the two std::string append helpers and accumulate the text in Python:
    #       0x10009c40(ecx=dest, char* literal)        -> append C-string
    #       0x10009c20(ecx=dest, std::string* src)     -> append std::string
    #     The MAIN VHDL string is the dest (ecx) that accumulates the most. We log
    #     (dest, bytes) per call and reconstruct afterwards.
    appends = []   # list of (dest_addr, src_bytes)

    def read_cstr(addr, maxn=0x40000):
        out = bytearray()
        while len(out) < maxn:
            try: chunk = bytes(uc.mem_read(addr+len(out), 64))
            except: break
            z = chunk.find(b"\x00")
            if z >= 0: out += chunk[:z]; break
            out += chunk
        return bytes(out)

    def read_stdstr(objaddr):
        # MSVC std::string: [0..0xf] SSO buffer / ptr@0, size@0x10, cap@0x14
        try:
            size = u32(objaddr+0x10); cap = u32(objaddr+0x14)
        except: return b""
        if cap > 0x4000000 or size > 0x4000000: return b""
        if cap < 16:
            return bytes(uc.mem_read(objaddr, size))
        ptr = u32(objaddr)
        try: return bytes(uc.mem_read(ptr, size))
        except: return b""

    def hook_append_cstr(uc, address, size, ud):   # 0x10009c40(this, char*)
        ecx = uc.reg_read(UC_X86_REG_ECX)
        sp = uc.reg_read(UC_X86_REG_ESP)
        src = u32(sp+4)                              # stack arg = char*
        appends.append((ecx, read_cstr(src)))

    def hook_append_str(uc, address, size, ud):     # 0x10009c20(this, std::string*)
        ecx = uc.reg_read(UC_X86_REG_ECX)
        sp = uc.reg_read(UC_X86_REG_ESP)
        srcobj = u32(sp+4)                           # stack arg = std::string*
        appends.append((ecx, read_stdstr(srcobj)))

    uc.hook_add(UC_HOOK_CODE, hook_append_cstr, begin=0x10009c40, end=0x10009c40)
    uc.hook_add(UC_HOOK_CODE, hook_append_str,  begin=0x10009c20, end=0x10009c20)

    # Plan A: with CRT lowio initialized the ofstream open should genuinely succeed,
    # so the two stream-state gates (0x100fc003 / 0x100fc11c) pass on their own and the
    # filebuf is fully built (no null-deref at 0x100c67d0). Leaving the gates UNPATCHED
    # is the correctness check; --force-gates restores the old je->jmp behaviour.
    PATCH_GATES = ("--force-gates" in sys.argv)
    if PATCH_GATES:
        uc.mem_write(0x100fc003, bytes([0xe9,0xdb,0x00,0x00,0x00,0x90]))
        uc.mem_write(0x100fc11c, bytes([0xe9,0xdb,0x00,0x00,0x00,0x90]))
        print("patched stream-state gates 0x100fc003 + 0x100fc11c je->jmp (force build)")
    else:
        print("gates UNPATCHED - relying on genuine ofstream open success")

    # ---- trace the REAL file-open subtree (this ofstream uses the wide variant) ----
    #   filebuf::open 0x101d6f46 -> 0x101d7017 -> _Fiopen(wide) 0x101da864
    #     -> _getstream 0x101e83ab  &  _wopenfile 0x101ec14c -> open-core 0x101fb492
    OPEN_TRACE = {"fiopen":0, "wopenfile":0, "opencore":0}
    OPEN_LOG = {"on": False, "blocks": []}
    def _t_fiopen(uc, address, size, ud):
        OPEN_TRACE["fiopen"] += 1
        OPEN_LOG["on"] = True; OPEN_LOG["blocks"] = []
        print("    [_Fiopen-wide 0x101da864] call #%d (block-trace ON)" % OPEN_TRACE["fiopen"])
    def _t_fiopen_exit(uc, address, size, ud):
        eax = uc.reg_read(UC_X86_REG_EAX)
        OPEN_LOG["on"] = False
        tag = "OK FILE*=0x%x" % eax if eax else "FAIL (NULL)"
        print("    [_Fiopen-wide] exit -> %s ; blocks=%s" %
              (tag, [hex(b) for b in OPEN_LOG["blocks"]]))
    def _t_wopenfile(uc, address, size, ud):
        OPEN_TRACE["wopenfile"] += 1
        print("    [_wopenfile 0x101ec14c] call #%d" % OPEN_TRACE["wopenfile"])
    def _t_opencore(uc, address, size, ud):
        OPEN_TRACE["opencore"] += 1
        print("    [open-core 0x101fb492] call #%d" % OPEN_TRACE["opencore"])
    def _t_getstream_exit(uc, address, size, ud):
        eax = uc.reg_read(UC_X86_REG_EAX)
        print("    [_getstream] exit eax=0x%x (0=NULL/EMFILE)" % eax)
    uc.hook_add(UC_HOOK_CODE, _t_fiopen,        begin=0x101da864, end=0x101da864)
    uc.hook_add(UC_HOOK_CODE, _t_fiopen_exit,   begin=0x101da91f, end=0x101da91f)
    uc.hook_add(UC_HOOK_CODE, _t_fiopen_exit,   begin=0x101da895, end=0x101da895)
    uc.hook_add(UC_HOOK_CODE, _t_wopenfile,     begin=0x101ec14c, end=0x101ec14c)
    uc.hook_add(UC_HOOK_CODE, _t_opencore,      begin=0x101fb492, end=0x101fb492)
    uc.hook_add(UC_HOOK_CODE, _t_getstream_exit, begin=0x101e8490, end=0x101e8490)
    def _open_blk(uc, address, size, ud):
        if OPEN_LOG["on"] and len(OPEN_LOG["blocks"]) < 300:
            OPEN_LOG["blocks"].append(address)
    uc.hook_add(UC_HOOK_BLOCK, _open_blk)

    # CreateFile2 feature-gate (0x101df935): under emulation its cached probe can
    # return "available", so the UCRT CreateFile wrapper (0x101fabd0) tries
    # GetProcAddress("CreateFile2") -> 0 -> returns INVALID_HANDLE_VALUE and the
    # open fails. Force it to return 0 so the wrapper uses the legacy CreateFileW
    # path, which our shim handles. 0-arg, caller-cleans -> pop only the ret addr.
    def _force_legacy_createfile(uc, address, size, ud):
        sp = uc.reg_read(UC_X86_REG_ESP)
        ret = struct.unpack("<I", uc.mem_read(sp, 4))[0]
        uc.reg_write(UC_X86_REG_ESP, sp + 4)
        uc.reg_write(UC_X86_REG_EAX, 0)
        uc.reg_write(UC_X86_REG_EIP, ret)
    uc.hook_add(UC_HOOK_CODE, _force_legacy_createfile, begin=0x101df935, end=0x101df935)

    bigDataObj = u32(crop+8)   # display object (builder fcn.101a8370 uses [crop+8])
    # generator does display = arg0+0x10; allow shifting arg0 to align display
    if "--argoff" in sys.argv:
        off = int(sys.argv[sys.argv.index("--argoff")+1], 0)
        bigDataObj = (bigDataObj + off) & 0xffffffff
        print("    arg0 shifted by %d -> 0x%x" % (off, bigDataObj))
    print("\n--- data_emu gen: bigDataObj=0x%x (display=arg0+0x10=0x%x) ---" % (bigDataObj, bigDataObj+0x10))

    # ---- app-level "send/data mode" global [0x1028a530] ----
    # Section-builder 0x100c9780 branches on this; headless init leaves it 0, which
    # takes a bad default path -> null-deref at 0x100c67d0. It's a GUI setting, not
    # from the .rcvp, so we must set it ourselves. Valid: 0xd (13) or 0x19 (25).
    # Override with --mode 0xNN (default 0xd).
    MODE = 0xd
    if "--mode" in sys.argv:
        MODE = int(sys.argv[sys.argv.index("--mode")+1], 0)
    uc.mem_write(0x1028a530, struct.pack("<I", MODE))
    print("set mode global [0x1028a530] = 0x%x (setter 0x101028b0)" % MODE)

    # ---- diagnostics on section-builder 0x100c9780 dispatch ----
    DIAG = ("--diag" in sys.argv)
    if DIAG:
        FORCE_FLAG = ("--force-flag" in sys.argv)
        def _d_entry(uc, address, size, ud):
            ebx = uc.reg_read(UC_X86_REG_ECX)   # ecx=this at entry
            mode = u32(0x1028a530)
            obj = u32(ebx)
            gate = 0
            try: gate = uc.mem_read(obj+0xd650,1)[0]
            except: gate = -1
            # display fields: word@+0x3c=W, word@+0x3e=H, byte@+0x85=scan, +0x44 dword
            def rb(o):
                try: return uc.mem_read(obj+o,1)[0]
                except: return -1
            def rw(o):
                try: return struct.unpack("<H", uc.mem_read(obj+o,2))[0]
                except: return -1
            print("    [0x100c9780 ENTRY] this=0x%x mode=0x%x D=[this]=0x%x byte[+0xd650]=0x%x  W@3c=%d H@3e=%d scan@85=%d d44=0x%x"
                  % (ebx, mode, obj, gate, rw(0x3c), rw(0x3e), rb(0x85), u32(obj+0x44)))
            if FORCE_FLAG and gate == 0:
                uc.mem_write(obj+0xd650, b"\x01")
                print("      [FORCED byte[D+0xd650]=1]")
        def _d_good(uc, address, size, ud):
            print("    [0x100c97c1 GOOD PATH taken]")
        def _d_bad(uc, address, size, ud):
            print("    [0x100c9b71 BAD PATH taken]")
        uc.hook_add(UC_HOOK_CODE, _d_entry, begin=0x100c9780, end=0x100c9780)
        uc.hook_add(UC_HOOK_CODE, _d_good,  begin=0x100c97c1, end=0x100c97c1)
        uc.hook_add(UC_HOOK_CODE, _d_bad,   begin=0x100c9b71, end=0x100c9b71)
        def _d_setflag(uc, address, size, ud):
            edi = uc.reg_read(UC_X86_REG_EDI)
            al = uc.reg_read(UC_X86_REG_EAX) & 0xff
            ecx = uc.reg_read(UC_X86_REG_ECX)
            print("    [0x10095f95 SET byte[edi+0xd650]] edi=0x%x al=0x%x (ecx=0x%x bit16=%d)"
                  % (edi, al, ecx, (ecx>>16)&1))
        uc.hook_add(UC_HOOK_CODE, _d_setflag, begin=0x10095f95, end=0x10095f95)

    WRITES.clear(); OUTWRITES.clear()
    call(0x100fbe20, [bigDataObj], "gen(displayobj)", ecx=0x1028a680)
    print("    captured %d append calls" % len(appends))
    print("    open-trace: %s ; CreateFileW filenames=%s" % (OPEN_TRACE, CREATEFILE_CALLS))

    # ---- PRIMARY capture: scan the heap for the fully-assembled VHDL ----
    # The filebuf is fully buffered (flushes only at close), and the close-flush
    # crashes in locale/codecvt conversion -> nothing reaches WriteFile. But the
    # complete assembled file (template with every #-PLACEHOLDER-# replaced) is
    # sitting in a contiguous heap buffer that starts with the VHDL header. Scan
    # the heap arena for that header and dump the whole buffer.
    # RESULT = the assembled data-portion: generator appends every section (CARDAREA
    # via inline sprintf, gamma/tables via 0x100c9780) into one std::string. It's
    # fully built before the flush-crash. Scan the heap arena for its content.
    MARKS = [b"to the control area", b"elsif y = 93", b"--Write 8bit HDR"]
    print("    scanning heap 0x%x..0x%x for assembled RESULT..." % (HEAP_BASE, heap_ptr))
    scan = HEAP_BASE
    CHUNK = 0x100000
    blob = bytearray()
    while scan < heap_ptr:
        n = min(CHUNK, heap_ptr - scan)
        try: blob += bytes(uc.mem_read(scan, n))
        except: blob += b"\x00"*n
        scan += n
    found = []
    for MARK in MARKS:
        idx = 0
        while True:
            j = blob.find(MARK, idx)
            if j < 0: break
            # walk back to the contiguous-text start (first byte after a NUL run)
            s = j
            while s > 0 and blob[s-1] != 0:
                s -= 1
            # walk forward to end of contiguous text (first NUL)
            e = blob.find(b"\x00", j)
            if e < 0: e = len(blob)
            found.append((HEAP_BASE+s, bytes(blob[s:e])))
            idx = e
    print("    heap-scan: found %d candidate blob(s)" % len(found))
    if found:
        addr, vhdl_heap = max(found, key=lambda t: len(t[1]))
        outfn = "/home/muse/Desktop/LED/re/emu/data_emu_full.txt"
        open(outfn, "wb").write(vhdl_heap)
        nplace = vhdl_heap.count(b"#-")
        print("    BEST blob @0x%x  %d bytes  remaining placeholders=%d  ('elsif y='=%d)" %
              (addr, len(vhdl_heap), nplace, vhdl_heap.count(b"elsif y =")))
        print("    wrote %s" % outfn)
        print("    head: %r" % vhdl_heap[:160])
        print("    tail: %r" % vhdl_heap[-100:])

    # ---- PRIMARY capture: the ofstream WriteFile output (ground-truth VHDL) ----
    print("    WriteFile: %d calls, %d distinct handles" % (len(WRITES), len(OUTWRITES)))
    if OUTWRITES:
        bigh = max(OUTWRITES, key=lambda h: len(OUTWRITES[h]))
        vhdl_wf = bytes(OUTWRITES[bigh])
        print("    biggest output handle=0x%x  %d bytes" % (bigh, len(vhdl_wf)))
        outfn = "/home/muse/Desktop/LED/re/emu/data_emu.vhd"
        open(outfn, "wb").write(vhdl_wf)
        print("    wrote %s (%d bytes)" % (outfn, len(vhdl_wf)))
        print("    head: %r" % vhdl_wf[:200])
        if len(vhdl_wf) > 4000:
            print("    SUCCESS: looks like the full VHDL. tail: %r" % vhdl_wf[-160:])
    else:
        print("    no WriteFile output captured (open may still be failing)")

    # group by dest; the main VHDL string = dest with the largest total text
    from collections import OrderedDict
    bydest = OrderedDict()
    order  = OrderedDict()
    for i,(dest,src) in enumerate(appends):
        bydest.setdefault(dest, bytearray())
        bydest[dest] += src
        order.setdefault(dest, i)
    if bydest:
        main = max(bydest, key=lambda d: len(bydest[d]))
        vhdl = bytes(bydest[main])
        print("    main dest=0x%x  total_len=%d  (%d distinct dests)" %
              (main, len(vhdl), len(bydest)))
        outfn = "/home/muse/Desktop/LED/re/emu/data_emu_filled.txt"
        open(outfn,"wb").write(vhdl)
        print("    wrote %s" % outfn)
        print("    head: %r" % vhdl[:200])
        # dump EVERY section blob in generation order to sections/ for parsing
        import os as _os
        secdir = "/home/muse/Desktop/LED/re/emu/sections"
        _os.makedirs(secdir, exist_ok=True)
        for d,b in sorted(bydest.items(), key=lambda kv: order[kv[0]]):
            fn = "%s/sec_%06d_%08x.txt" % (secdir, order[d], d)
            open(fn,"wb").write(bytes(b))
        print("    wrote %d section blobs to %s/" % (len(bydest), secdir))
        # also dump all dests for inspection
        import json as _jg
        meta = [{"dest":hex(d), "len":len(b), "first_call":order[d],
                 "head":bytes(b)[:80].decode("latin1")} for d,b in bydest.items()]
        open("/home/muse/Desktop/LED/re/emu/gen_dests.json","w").write(_jg.dumps(meta, indent=1))
    return
